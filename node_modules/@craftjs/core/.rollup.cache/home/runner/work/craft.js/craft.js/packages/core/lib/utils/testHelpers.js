...
const getTestNode = (parentNode) => {
    const { events, data: { nodes: childNodes, linkedNodes }, ...restParentNode } = parentNode;
...
export const createTestNodes = (rootNode) => {
    const nodes = {};
    const iterateNodes = (testNode) => {
        ...
        if (childNodes) {
            childNodes.forEach((childTestNode, i) => {
                const { node: childNode, childNodes: grandChildNodes, linkedNodes: grandChildLinkedNodes, } = getTestNode(childTestNode);
        ...
        if (linkedNodes) {
            Object.keys(linkedNodes).forEach((linkedId) => {
                const { node: childNode, childNodes: grandChildNodes, linkedNodes: grandChildLinkedNodes, } = getTestNode(linkedNodes[linkedId]);
