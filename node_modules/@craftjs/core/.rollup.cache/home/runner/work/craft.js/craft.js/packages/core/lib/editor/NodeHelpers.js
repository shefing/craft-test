import { deprecationWarning, ERROR_CANNOT_DRAG, ERROR_DUPLICATE_NODEID, ERROR_INVALID_NODE_ID, ERROR_MOVE_INCOMING_PARENT, ERROR_MOVE_NONCANVAS_CHILD, ERROR_MOVE_OUTGOING_PARENT, ERROR_MOVE_TO_DESCENDANT, ERROR_MOVE_TO_NONCANVAS_PARENT, ERROR_MOVE_TOP_LEVEL_NODE, ERROR_MOVE_CANNOT_DROP, ROOT_NODE, } from '@craftjs/utils';
import invariant from 'tiny-invariant';
import { getNodesFromSelector } from '../utils/getNodesFromSelector';
import { serializeNode } from '../utils/serializeNode';
export function NodeHelpers(state, id) {
    ...
    return {
        ...
        ancestors(deep = false) {
            function appendParentNode(id, ancestors = [], depth = 0) {
                const node = state.nodes[id];
                if (!node) {
                    return ancestors;
                }
        ...
        isDroppable(selector, onError) {
            ...
            try {
                ...
                invariant(newParentNode.rules.canMoveIn(targets.map((selector) => selector.node), newParentNode, nodeHelpers), ERROR_MOVE_INCOMING_PARENT);
                const parentNodes = {};
                targets.forEach(({ node: targetNode, exists }) => {
                    ...
                    invariant(!nodeHelpers(targetNode.id).isTopLevelNode(), ERROR_MOVE_TOP_LEVEL_NODE);
                ...
                Object.keys(parentNodes).forEach((parentNodeId) => {
                    ...
                    invariant(parentNode.rules.canMoveOut(childNodes, parentNode, nodeHelpers), ERROR_MOVE_OUTGOING_PARENT);
                });
                return true;
            }
            catch (err) {
        ...
        toNodeTree(includeOnly) {
            const nodes = [id, ...this.descendants(true, includeOnly)].reduce((accum, descendantId) => {
